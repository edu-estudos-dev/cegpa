...
class PesquisaController {
    ...
    async pesquisaPorSimilaridade(req, res) {
        ...
        try {
            ...
            res.status(500).json({
                error: 'Erro na pesquisa por similaridade.',
            });
        }
    }
    ...
    async fetchItensNaoPagosPorCategoria(req, res) {
        ...
        try {
            ...
            console.error(
                'Erro ao buscar itens não pagos por categoria:',
                error
            );
            res.status(500).json({
                error: 'Erro ao buscar itens não pagos por categoria.',
            });
        }
    }
    ...
    async fetchItensNaoPagosPorSubgrupo(req, res) {
        ...
        try {
            const quantidadeNaoPagos =
                await PesquisaModel.getItensNaoPagosPorSubgrupo(subgrupo);
            console.log(
            ...
            console.error(
                'Erro ao buscar itens não pagos por subgrupo:',
                error
            );
            res.status(500).json({
                error: 'Erro ao buscar itens não pagos por subgrupo.',
            });
        }
    }
    ...
    fetchItensSaidosPorAno = async (req, res) => {
        ...
        try {
            ...
            const quantidadeNoEstoque =
                await PesquisaModel.getQuantidadeNoEstoque(categoria || ''); // Corrige para buscar quantidade no estoque
            res.json({ quantidadeSaidos, quantidadeNoEstoque });
        } catch (error) {
            console.log('Erro ao buscar itens saídos por ano:', error);
            res.status(500).json({
                error: 'Erro ao buscar itens saídos por ano.',
            });
        }
    };
    ...
    fetchItensEntradaPorAno = async (req, res) => {
        ...
        if (!qtd_itens_entraram) {
            ...
            return res
                .status(400)
                .json({
                    error: 'Quantidade de itens de entrada não fornecida.',
                });
        }
        ...
        try {
            ...
            res.status(500).json({
                error: 'Erro ao buscar itens entraram por ano.',
            });
        }
    };
